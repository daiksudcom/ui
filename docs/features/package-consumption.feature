...
Feature: UI パッケージを独立したバージョンで利用する
  ...
  Rule: 公開パッケージは再現可能な利用契約を提供する
    ...
    Scenario: 公開エクスポートを導入する
      Given GitHub Packages に "@daiksudcom/ui" のバージョン "2.3.1" が公開されている
      When 利用者が "@daiksudcom/ui@2.3.1" を導入する
    ...
    Scenario: Home と Blog が異なるバージョンを選択する
      Given Home は "@daiksudcom/ui@2.3.1" を厳密に指定している
      And Blog は "@daiksudcom/ui@2.4.0" を厳密に指定している
