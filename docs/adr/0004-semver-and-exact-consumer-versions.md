...
resource: "https://github.com/daiksudme/ui/blob/main/docs/adr/0004-semver-and-exact-consumer-versions.md"
...
`@daiksudme/ui` は `0.1.0` からSemVerで公開する。`0.x` 中のbreaking changeはminor、安定した公開契約を宣言するときは `1.0.0`、以後のbreaking changeはmajorとする。Home と Blog は範囲指定ではなく厳密なバージョンを記録し、それぞれの受け入れ検証を経て個別に更新する。公開済み成果物は不変とする。
